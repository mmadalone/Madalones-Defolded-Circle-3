...
**Custom firmware. Voids the warranty.** 

Fork of [`unfoldedcircle/remote-ui`](https://github.com/unfoldedcircle/remote-ui) for the **Unfolded Circle Remote 3**, replacing the stock analog-clock charging screen with a GPU-accelerated screensaver system.
...
**UI chrome beyond the screensaver (v1.3.0 → v1.4.43):**

- **Tap-to-close semantics fix + "Chime after screensaver" toggle (pattern B)** (v1.4.43) — Two screensaver-UX fixes layered on v1.4.42. Single-tap now dismisses the screensaver when "Double-tap to close" is OFF (was: tap had no effect in normal mode + a 300 ms delayed Matrix tap-effect glitch fired via the doubleTapTimer's onTriggered handler — bug fixed for free since the timer no longer starts when tapToClose is off). Plus a new **`Settings → Sound → "Chime after screensaver"`** toggle (default ON): screensaver opens immediately on dock, chime fires from `ChargingScreen.qml::onOpened` once the popup is fully shown. By that point all heavy QML/GPU init is done, so the audio thread has clean CPU and chimes play without ALSA buffer underruns. Dock-to-audio latency goes from 0 ms (legacy) to ~450 ms (pattern B), but visual + audio synchronize at ~450 ms instead of waiting up to 3 s for long chimes (Pwr Up 1 etc) to finish first. Toggle OFF preserves v1.4.42's chime-first + variant-aware-grace path. Boot-while-docked unchanged regardless of toggle. Wiring: `Config.dockChimeAfterScreensaver` Q_PROPERTY, `_nextOpenViaDock` flag in `main.qml` propagated through `Loader.onStatusChanged → ChargingScreen.qml::_openedViaDock`. Refreshed `tos_bridge_loss_power_shorter.wav` source (1.86 → 1.24 s, RMS 0.122 → 0.432 through pipeline); `_chimeDurationsMs[10]` updated to match.
...
- **Mod 5 v3: ping fallback removed** (v1.5.1) — 17-day soak across UCR3 firmware 2.9.1 → 2.9.2 → 2.9.3 verified the REST inhibitor path is reliable in production. The legacy WS ping fallback (kept as a safety net in v1.4.39) is removed: ~120 LOC net delete, one Q_PROPERTY gone (`Config.sessionKeeperUseInhibitorApi`), one Settings toggle removed. Single effector contract now — keeper-on means inhibitor-on. Rationale: the fallback was never exercised on real firmware after v1.4.39, re-introduced the exact race the v2 swap fixed if invoked, and obscured the silent-failure mode that bit Mod 5 v1 once already (the `setPowerMode` body field bug; see `project_setpowermode_field_bug.md`). Auto-revert remains the real safety net if a future firmware breaks REST.
- **Mod 5 v2: standby_inhibitors REST swap** (v1.4.39) — Active Session Keeper's effector switches from the legacy 270 s `setPowerMode(NORMAL)` WS ping loop to the firmware's native `POST /api/system/power/standby_inhibitors` REST endpoint. Event-based contract eliminates the 270 s vs 300 s race window where session activity stopping just before a ping cycle could let the firmware's standby timer expire and nuke the session. Bonus side-effect: BLOCK inhibitor pins the device in NORMAL during sessions, so the `LOW_POWER ↔ NORMAL` transitions that trigger phantom-wake events can't fire while a session is running — Mod 6 still owns idle-period suppression. Authenticated via `Authorization: Bearer <UC_TOKEN content>` (Phase 0 REST auth probe established this is the firmware's accepted REST auth contract on UCR3 firmware 2.9.1, despite Bearer not being declared in OpenAPI `securitySchemes`). Initially shipped behind a `Config.sessionKeeperUseInhibitorApi` toggle (default ON); toggle removed in v1.5.1 after soak verification.
...
- **Mod 5 + Mod 6 force-back API fix + wake-press detection** (v1.4.22, v1.4.23) — critical fix: `Api::setPowerMode` WS RPC body field was `"power_mode"` but firmware's serde expects `"mode"`, so every Mod 5 ping (since v1.4.14) and every Mod 6 force-back (since v1.4.20) failed silently with HTTP 400. v1.4.22 fixes the field name, adds `PhantomWakeSuppressor::onEntityCommandIssued` slot (firmware delivers wake-press events directly to integrations, bypassing `inputController::keyPressed`, so the entity-command layer is the only reliable signal). v1.4.23 adds `m_lastInputTimer` recent-input lookback (default 500 ms, range 0–2000) — handles the firmware-side delivery ordering where wake-press signals fire BEFORE `Power::powerModeChanged` on wake-from-LOW_POWER. Grace window range expanded from 100–2000 ms to 100–5000 ms.
- **Phantom-Wake Suppressor (Mod 6)** (v1.4.20, fixed v1.4.22 + v1.4.23) — Settings → Power "Suppress phantom wake-ups". Inverse-symmetric companion to Mod 5: detects `LOW_POWER|SUSPEND → NORMAL` transitions and arms a configurable grace timer (default 500 ms, range 100–5000). User-input signals during grace cancel the timer (`InputController::keyPressed` / `touchDetected`, `TouchSlider::touchPressed`, `Battery::powerSupplyChanged`, `EntityController::entityCommandIssued` with curated allowlist). The recent-input lookback skips arming if user input arrived in the last 500 ms (configurable 0–2000 ms) before the wake transition — handles firmware delivery ordering. If grace expires without input, fires `core::Api::setPowerMode(LOW_POWER)` to force the device back to standby. Eliminates the ~9 %/hr off-dock battery drain caused by WoWLAN-induced phantom wakes. Default off, opt-in.
...
- **Active Session Keeper** (v1.4.14, fixed v1.4.22) — Settings → Power "Keep awake while watching/listening". Resets the firmware's sleep countdown to its configured max via periodic `set_power_mode(NORMAL)` pings every 4.5 min while a media-player entity is in `Playing` state or curated media-control buttons (`PLAY_PAUSE` / `STOP` / `VOLUME_*` / `CURSOR_*` / `CHANNEL_*` / `NEXT` / `PREVIOUS` / etc.) have been pressed within a configurable idle window (30–300 s). Wires the previously-orphan `set_power_mode` ucapi RPC. Default off, AC-required default on. Eliminates the 1–3 s wake-recovery gap on every sleep cycle while watching TV. **Note:** the v1.4.22 release fixed a silent failure that had been present since v1.4.14 — the WS RPC body field name was wrong, so every ping had been returning HTTP 400; the keeper was a no-op throughout v1.4.14 → v1.4.21. Battery/UX improvements promised by Mod 5 only landed in production with v1.4.22.
...
- **MediaBrowser → player-widget thumbnail preview handoff** (v1.4.9) — tapping an item in the MediaBrowser popup now passes its browse-time thumbnail to the `MediaPlayer` entity as an immediate preview, rendering on the widget before the integration's `Player.GetItem` art response arrives. Bridges the "blank player widget post-tap" gap for library content that integrations CAN surface. Scheme-filter on `setPreviewImage` rejects unfetchable `icon://` / `image://` / non-image-data URIs at entry to avoid `ProtocolUnknownError` retry-burn when the integration falls back to an icon-name. Paired with empty controls-bar auto-collapse: when all four v1.4.8 button toggles are off, the 80 px controls row collapses instead of reserving an empty gap under the progress bar. Coverage of unscraped library content depends on the integration's browse response — separate integration-side work tracks the `art.thumb` / `Files.GetDirectory` vs `VideoLibrary.GetMovies` resolution gap.
...
curl -L -O https://github.com/mmadalone/Madalones-Defolded-Circle-3/releases/download/v1.4.43/remote-ui-v1.4.43-UCR3-static.tar.gz
curl -L -O https://github.com/mmadalone/Madalones-Defolded-Circle-3/releases/download/v1.4.43/remote-ui.hash

# 2. Verify integrity (SHA256 + GPG if signed — see docs/RELEASE_SIGNING.md)
./scripts/verify-release.sh remote-ui-v1.4.43-UCR3-static.tar.gz remote-ui.hash
...
curl --location "http://${UC3_HOST}/api/system/install/ui?void_warranty=yes" \
    --form "file=@remote-ui-v1.4.43-UCR3-static.tar.gz" \
    -u "web-configurator:${UC3_PIN}"
```
...
| Layer | Where | What |
|---|---|---|
| **C++ renderers** (`src/ui/`) | `matrixrain.cpp` (QQuickItem + QSGGeometryNode + texture atlas, orchestrator) + `src/ui/matrixrain/` 5 helper classes (LayerPipeline, AtlasBuilder, SingleLayerRenderer, InputHandler, BindingHelper — see `SCREENSAVER-IMPLEMENTATION.md`), `rainsimulation.cpp` (simulation logic), `glyphatlas.cpp` (GPU glyph atlas) | GPU pipeline, simulation tick, per-frame vertex emission on the Qt scene-graph render thread |
...
./scripts/verify-release.sh \
    remote-ui-v1.4.43-UCR3-static.tar.gz \
    remote-ui.hash \
    remote-ui-v1.4.43-UCR3-static.tar.gz.asc
```
...
**v1.4.18** (2026-04-29) — CI fix: bump `remote-ui.pro:75` `VERSION` from 1.4.11 → 1.4.18. Six releases (v1.4.12-v1.4.17) silently failed the `.github/workflows/build.yml:44-55` consistency check between `remote-ui.pro` and `release.json`. No runtime impact; firmware-side displayed version comes from `GIT_VERSION` (`git describe --tags`). CI artifact build was the only consumer of the bumped line.

**v1.4.17** (2026-04-29) — WiFi Diagnostics popup (W13). New "Diagnostics" button in the WifiInfo popup action stack opens a focused diagnostic surface with: RSSI sparkline (Canvas, last 60 samples, dashed reference lines at SignalStrength tier thresholds -60/-76/-84 dBm), live RSSI + link speed, drops since boot, current session uptime (HH:MM:SS, 1 Hz tick), time since last drop, Reset counters button (with confirmation dialog). New custom QML file `WifiDiagnostics.qml` (~280 lines). C++ surface: 4 new Q_PROPERTYs on Wifi singleton (`rssiHistory` QVariantList, `disconnectCount`, `currentSessionDurationSec`, `secondsSinceLastDisconnect`) + ring buffer `m_rssiHistory` (capped at 60) + `Q_INVOKABLE resetDiagnosticCounters()`. 5 s fast poll while popup open, 30 s background W6 poll otherwise; both push to the same buffer.
...
State of the world at v1.4.19. Items here aren't TODOs in the imperative sense — they're "if I ever care, this is where I left it."

**Validation pending (no code, just observation):**
- **Mod 5 Active Session Keeper end-to-end** during real TV watching. Toggle visibility confirmed; the 30-min movie keeping device pinned to NORMAL through a continuous Playing state hasn't been observed long-form.
- **W6** 30 s periodic `getWifiStatus` poll visibility in Logdy.
- **W10** scan-timer halt on screen-off in Logdy.
- **W9** onboarding scoped failure cleanup — only triggerable on factory reset, so untested.
- **WoWLAN day-long battery-impact** observation (per `feedback_ha_entity_prune_thermal.md` memory note).
- **v1.4.19 LOW_POWER trigger fix** in real use — the bigger functional change of v1.4.19 is silently activating the `m_pendingCommands` retry loop that had been a no-op on UCR3. Whether it actually solves the "first press post-wake doesn't land" complaint is the test.

**Backlog (low-priority, deferrable):**
- **W7** — WpaState progression UI in connect flow. Helps debug stuck-handshake / wrong-password scenarios with "Authenticating…" / "4-way handshake" intermediate labels. Niche; rare event.
- **W8** — proper WPA3/EAP scan-result auth parsing. `wifi.cpp:177` has `// TODO(marton): properly convert security` and force-casts every encrypted result to `WPA2_PSK`. Visual-only bug; lock icon is correct, the security enum lies for WPA3 networks.
- **W11** — WPA3 / Enterprise option in the "Join other" wizard. Corporate-network scenario.
- **W14** — WiFi quick-toggle in notification drawer (phone-style control center). ~80 LOC.

**Skipped (covered or duplicative):**
- **W12** — per-network auto-connect toggle. Already covered by Enable/Disable in `WifiNetworkList.qml` known-networks list.

**Deferred concepts (no plan, no commitment):**
- Per-integration "Reconnecting to {X}…" labels on the Wake-replay HUD (Stage 4 of the v1.4.19 plan). Would require new `Q_PROPERTY` on `IntegrationController`. Generic banner is sufficient when WiFi-level reconnect is the dominant failure mode.
- `docs/CUSTOM_FILES.md` per-Mod sections rewrite. Currently v1.4.11 baseline; only the v1.4.12+ summary table is current. Skip until a contributor needs the per-Mod detail.
- Mod 6+. No specific candidate queued.
