...
**Date:** 2026-05-20 (manual on-device pass — N7 closure)
**Auditor:** Claude Opus 4.7 (1M context), guiding madalone through the §2-§3 checklist on hardware
**Firmware build:** UC core `0.73.1-bt` / OS `2.9.3` / custom UI `v1.5.1` (commit `b8fdb28`)
**Device:** maintainer's UCR3 (192.168.2.204), docked on AGPTEK 15W Qi pad

### Methodology

Manual driver-and-observer pass: assistant dictated each checklist item in 5 batches (1A/1B/1C — §2.1 DPAD nav across all 12 pages; 2 — §2.3 touch targets; 3 — §2.4 theme overlays; 4 — §2.5 dynamic bindings; 5 — §3 cross-batch regressions). User executed each step on the device and reported pass/fail. German locale (`de_DE`) set for batches 1A-1C to incidentally cover §2.2 translation overflow on upstream chrome while doing the DPAD sweep; English restored for the remainder.

### Findings

| Section | Status | Notes |
|---|---|---|
| §2.1 Focus & DPAD wiring | ✅ pass on device | All 12 settings pages (ChargingScreen parent + 11 sub-pages) reach every interactive item via DPAD up/down. Focus indicator visible throughout. Left/Right behaves correctly (no-op or in-row cycle, never an accidental action). Scroll smooth, Return goes back one level. **The static pre-pass concern about `AnalogSettings.qml` having only 2 `KeyNavigation` refs is RESOLVED** — user verified on device that all items are reachable. The 2-vs-8-26 discrepancy reflects that AnalogSettings has fewer items, not a sparse chain. |
| §2.2 Text & contrast | ✅ pass (implicit via German locale) | 45-min navigation in `de_DE` through all 12 pages produced no overflow flags on upstream UC chrome (page titles, navigation labels, back-button). No too-small-text concerns surfaced during the §2.1 sweep glance. **Translation coverage gap noted as a separate finding (non-blocking nit, see below).** |
| §2.3 Touch targets | ✅ pass | Tap slop verified within ~8px on toggle rows and Theme Selector tiles. Double-tap protection working — screensaver double-tap closes once cleanly, no Matrix tap-effect glitch firing at 300ms (the v1.4.43 fix verified in production). Single-tap-when-tapToClose-OFF closes immediately (v1.4.43 fix's other half). |
| §2.4 Theme overlays | ✅ pass on all 5 themes | Matrix (including rainbow color mode worst-case) — clock + battery readable. Starfield, Minimal (date line fits at max `minimalDateSize`), Analog (second hand visible at idle-visible state), TV Static (max noise intensity does NOT obscure overlays). Shared `Battery text size` slider verified at min/default/max — no truncation at any value. |
| §2.5 Overlay bindings | ✅ pass | `showClock` OFF — clock fully hidden in Matrix + Minimal (spot-check). `showBatteryEnabled` OFF — battery fully hidden in both. Wake from display-off re-renders screensaver instantly with overlays intact (folds Bug 2 + Bug 2b from §3 into this category). Undock with "Close on wake" OFF preserves overlays. |
| §3 Cross-batch regressions | ✅ all 7 pass | **Bug 1**: Close on wake ON closes on undock, OFF keeps screensaver visible (both states verified). **Bug 2 + 2b**: wake from display-off instant, undock-to-display-off gap < 2 sec (no 7-sec regression). **Bug 3**: Idle screensaver OFF prevents auto-open, ON opens after configured timeout. **DPAD direction regression**: Matrix rain bends smoothly via gravity lerp on direction input — no respawn. **Thermal**: 10+ min sustained dock with Matrix + rainbow color mode produced no noticeable warmth (hand-feel). **AnalogTheme first-wake**: clock hands visible (not black/transparent) — Qt.binding pre-conversion workaround holds. |
...
- **Translation coverage gap on custom screensaver strings.** German locale falls back to English for all sub-pages under `Settings → Screensaver` because the `qsTr()`-wrapped strings have `<translation type="unfinished"></translation>` entries in `resources/translations/*.ts` — lupdate picks them up, no human translator has filled them in. This is expected behavior (no German speaker has worked the translations) and not a code defect. **Implication for future audits:** German locale's §2.2 overflow check only covers upstream UC chrome, not our custom strings. If a future translation pass fills the German strings, re-run §2.2 to catch any overflow that surfaces.
- **`MatrixTheme.qml:124` debug atlas overlay font 9px** still flagged from the static pre-pass. Confirmed not user-facing in production (toggle hidden); no change in on-device pass.

### Audit closure

This on-device pass closes N7 from `audit-v1.4.26-thorough.md` and `project_path_to_a_post_v1_4_35.md`. The path-to-A− roadmap's terminal gate is now passed. **Effective grade: A−.**

The 12 settings pages, 5 themes, and 7 regression tests are all behaving as designed on UCR3 firmware 2.9.3 running custom UI v1.5.1. No follow-up work generated by this audit.

---

### Prior audit (preserved per §4 append convention)
