...
| `deploy/config/click.wav` `click_lo.wav` `confirm.wav` `error.wav` `zap_future.wav` | **Unreleased.** CC0 substitute UI sound effects, 16-bit PCM stereo at 44.1 kHz, ~140 KB total. Bundled because firmware 2.9.2 dropped `UC_SOUND_EFFECTS_PATH`; `uiController.cpp` falls back to `$UC_CONFIG_HOME` (= this dir) when the env is empty. Stereo required — UCR3's ALSA backend rejects mono with `snd_pcm_hw_params_set_channels: err = -22`. Generated by `tools/gen_sounds.py` (regen any time; replaceable with stock UC originals if obtained, same filenames). Flat layout — `./config/` cannot contain sub-directories per the install endpoint's archive validator. |
| `tools/gen_sounds.py` | **Unreleased.** Numpy-based generator for the 5 CC0 wav files above. Runs offline; output goes directly to `deploy/config/`. ~110 LOC, pure-Python with numpy dependency. |
...
| `src/ui/soundEffects.cpp` | **v1.4.6 (path/file guards) + Unreleased (diagnostic loglevel bump).** v1.4.6: added `#include <QFileInfo>`; initialized all 5 `QSoundEffect*` members to `nullptr` in the ctor init list; added empty-path guard + file-existence guard in `createEffects()` (new `makeEffect` lambda skips `setSource` when the path is empty or the file doesn't exist) and null-guards around every `m_effect*->setVolume()` / `->play()` call in `play()`'s switch. Prevents 5× "QSoundEffect(qaudio): Error decoding source file:///*.wav" warnings when `UC_SOUND_EFFECTS_PATH` env var is unset or firmware-provided sound files are missing. **Unreleased**: rerouted the 5 diagnostic `qCDebug(lcUi())` calls to `qCWarning(lcCore())` — bare `uc.ui` is silenced before journald in firmware 2.9.x, and `qCDebug` is dropped entirely; only WARN+ uc.core surfaces. The diagnostic stays in place as a future-regression detector — `/api/system/logs?s=custom-ui&q=Default+audio` shows `"default"` (or whatever the audio sink is); empty `q=UC_SOUND` query means the env-empty fallback worked or the env was set; `q=file+missing` should be empty if the bundled wavs are reachable. |
| `src/ui/uiController.cpp` | **Newly modified by Unreleased.** Added `UC_SOUND_EFFECTS_PATH` empty-fallback at the `SoundEffects` constructor call site (line 71). When the env var is empty (firmware 2.9.2 regression), reads `qgetenv("UC_CONFIG_HOME")` instead — same env our font (`glyphatlas.cpp:51`) and screensaver json (`screensaverconfig.cpp:21`) already use. Pairs with the bundled wavs in `deploy/config/` (flat layout — firmware install endpoint rejects nested dirs under `./config/` with `400 "Could not extract archive"`). Env still wins if non-empty, so future firmware that restores `UC_SOUND_EFFECTS_PATH` is forward-compatible. Net delta: ~9 LOC. |
